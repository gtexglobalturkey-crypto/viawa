...
export function createGoogleWorkspaceClient(input: {
  ...
  async function copyMaster(name: string) {
    ...
    if (input.requireProviderUrls) {
      const url = new URL(data.webViewLink ?? "https://invalid.invalid");
      if (url.protocol !== "https:" || url.hostname !== "docs.google.com" || url.username || url.password
        || url.pathname.split("/")[3] !== data.id || !url.pathname.startsWith("/document/d/")) throw new Error("GOOGLE_COPY_URL_MISMATCH");
    }
  ...
  async function verifyPlaceholders(targetId: string) {
    ...
    const unresolved = findDisallowedUnresolvedPlaceholders(collectText(JSON.parse(new TextDecoder().decode(await readBoundedBytes(response, 5 * 1024 * 1024)))));
    if (unresolved.length) throw new Error(`UNRESOLVED_CONTRACT_PLACEHOLDERS:${unresolved.join(",")}`);
  }

  async function exportPdf(targetId: string): Promise<Uint8Array> {
    assertCopyTarget(input.masterTemplateId, targetId);
    ...
    const pdf = await readBoundedBytes(response, 10 * 1024 * 1024);
    if (pdf.length < 6 || new TextDecoder().decode(pdf.subarray(0, 5)) !== "%PDF-") throw new Error("INVALID_GOOGLE_PDF_EXPORT");
    return pdf;
  }

  async function uploadPdf(name: string, pdf: Uint8Array) {
    ...
    if (input.requireProviderUrls) {
      ...
      if (url.protocol !== "https:" || url.hostname !== "drive.google.com" || url.username || url.password
        || !url.pathname.startsWith(`/file/d/${data.id}/`)) throw new Error("GOOGLE_PDF_URL_MISMATCH");
    }
